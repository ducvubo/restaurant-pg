...
import { deleteCategory, restoreCategory } from '../category.api'
...
export default function DeleteOrRestore({ inforCategory, path }: Props) {
  ...
  const handleDeleteCategory = async (_id: string) => {
    setLoading(true)
    const res = path === 'recycle' ? await restoreCategory({ _id }) : await deleteCategory({ _id })
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {path === 'delete'
              ? `Bạn có chắc muốn chuyển danh mục '${inforCategory.cat_res_name}' vào thùng rác không?`
              : `Bạn có chắc muốn khôi phục danh mục '${inforCategory.cat_res_name}' không?`}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>Không</AlertDialogCancel>
          <AlertDialogAction onClick={() => handleDeleteCategory(inforCategory._id)}>Có</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  )
