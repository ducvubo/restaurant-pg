...
export function PageFoodCombos<TData, TValue>({ columns, meta, data }: DataTableProps<TData, TValue>) {
  ...
  React.useEffect(() => {
    router.push(`/dashboard/food-combos/${pathname === 'recycle' ? 'recycle' : ''}?page=${pageIndex}&size=${pageSize}`)
  }, [pageIndex, pageSize, router])

  return (
    <div className='flex flex-col h-[570px]'>
      <div className='flex justify-end gap-2 items-center py-4'>
        <Button variant={'outline'}><Link href={'/dashboard/food-combos/add'}>Thêm</Link></Button>
        <Button variant={'outline'}><Link href={'/dashboard/food-combos/recycle'}  >Danh sách đã xóa</Link></Button>
