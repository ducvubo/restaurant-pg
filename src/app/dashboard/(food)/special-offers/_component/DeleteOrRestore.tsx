...
export default function DeleteOrRestore({ inforSpecialOffer, path }: Props) {
  ...
  const handleDeleteSpecialOffer = async (spo_id: string) => {
    ...
    if (res.statusCode === 200) {
      ...
      toast({
        title: 'Thành công',
        description:
          path === 'recycle' ? 'Khôi phục thành công' : 'Chuyển vào thùng rác thành công',
        variant: 'default'
      })
      router.refresh()
    } else if (res.code === -10) {
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{path === 'delete' ? 'Xóa ưu đãi' : 'Khôi phục ưu đãi'} </AlertDialogTitle>
          <AlertDialogDescription>
            {path === 'delete'
              ? `Bạn có chắc muốn chuyển ưu đãi này vào thùng rác không?`
              : `Bạn có chắc muốn khôi phục ưu đãi này không?`}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>Không</AlertDialogCancel>
          <AlertDialogAction onClick={() => handleDeleteSpecialOffer(inforSpecialOffer.spo_id ? inforSpecialOffer.spo_id : '')}>
            Có
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
