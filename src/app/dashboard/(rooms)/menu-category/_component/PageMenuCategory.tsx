...
export function PageMenuCategory<TData, TValue>({ columns, meta, data }: DataTableProps<TData, TValue>) {
  ...
  React.useEffect(() => {
    router.push(
      `/dashboard/menu-category/${pathname === 'recycle' ? 'recycle' : ''}?page=${pageIndex}&size=${pageSize}`
    )
    ...
    <div className='flex flex-col' style={{ height: 'calc(100vh - 7rem)' }}>
      <div className='flex justify-end gap-2 items-center py-4'>
        <Button variant={'outline'}>
          <Link href={'/dashboard/menu-category/add'}>Thêm</Link>
        </Button>
        <Button variant={'outline'}>
          <Link href={'/dashboard/menu-category/recycle'}>Danh sách đã xóa</Link>
        </Button>
