...
export function PageInternalProposal<TData, TValue>({ columns, meta, data }: DataTableProps<TData, TValue>) {
  ...
  React.useEffect(() => {
    router.push(
      `/dashboard/internal-proposal/${pathname === 'recycle' ? 'recycle' : ''}?page=${pageIndex}&size=${pageSize}`
    )
  ...
  const debouncedSearch = React.useCallback(
    debounce((value: string) => {
      router.push(
        `/dashboard/internal-proposal/${pathname === 'recycle' ? 'recycle' : ''
  ...
  return (
    <div className='flex flex-col' style={{ height: 'calc(100vh - 7rem)' }}>
      <div className='flex justify-end gap-2 items-center py-4'>
        ...
        <Button variant={'outline'} disabled={!hasPermissionKey('internal_proposal_create')}>
          <Link href={'/dashboard/internal-proposal/add'}>Thêm</Link>
        </Button>
        {
          pathname === 'recycle' ? (
            <Button variant={'outline'} disabled={!hasPermissionKey('internal_proposal_view_list')}>
              <Link href={'/dashboard/internal-proposal'}>Danh sách</Link>
            </Button>
          ) : (
            <Button variant={'outline'} disabled={!hasPermissionKey('internal_proposal_view_deleted')}>
              <Link href={'/dashboard/internal-proposal/recycle'}>Danh sách đã xóa</Link>
            </Button>
          )
        }
