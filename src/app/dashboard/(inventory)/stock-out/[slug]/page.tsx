...
import { findStockOutById, getAllStockOuts } from '../stock-out.api'
...
async function Component({ searchParams, params }: PageProps) {
  ...
  if (id === 'add') {
    return (
      <ContentLayout title='Thêm phiếu xuất'>
        <AddOrEdit id='add' />
      </ContentLayout>
    )
  }
  ...
  if (id === 'recycle') {
    ...
    if (res.statusCode === 200) {
      ...
      return (
        <div>
          <ContentLayout title='Danh sách phiếu xuất đã xóa'>
  ...
  const res: IBackendRes<IStockOut> = await findStockOutById({ stko_id: id })
  ...
  return (
    <ContentLayout title='Chỉnh sửa thông tin phiếu xuất'>
      <AddOrEdit id={id} inforStockOut={res.data} />
    </ContentLayout>
  )
}
