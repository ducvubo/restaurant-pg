...
export function PageCatIngredient<TData, TValue>({ columns, meta, data }: DataTableProps<TData, TValue>) {
  ...
  const debouncedSearch = React.useCallback(
    debounce((value: string) => {
      ...
      router.push(
        `/dashboard/cat-ingredients/${pathname === 'recycle' ? 'recycle' : ''
  ...
  return (
    <div className='flex flex-col' style={{ height: 'calc(100vh - 7rem)' }}>
      <div className='flex justify-end gap-2 items-center py-4'>
        ...
        <Button variant={'outline'} disabled={!hasPermissionKey('cat_ingredient_create')}>
          <Link href={'/dashboard/cat-ingredients/add'}>Thêm</Link>
        </Button>
        {
          pathname === 'recycle' ? (
            <Button variant={'outline'} disabled={!hasPermissionKey('cat_ingredient_view_list')}>
              <Link href={'/dashboard/cat-ingredients'}>Danh sách</Link>
            </Button>
          ) : (
            <Button variant={'outline'} disabled={!hasPermissionKey('cat_ingredient_view_deleted')}>
