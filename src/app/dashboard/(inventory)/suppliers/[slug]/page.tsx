...
import { findSupplierById, getAllSuppliers } from '../supplier.api'
...
async function Component({ searchParams, params }: PageProps) {
  ...
  if (id === 'add') {
    return (
      <ContentLayout title='Thêm nhà cung cấp'>
        <AddOrEdit id='add' />
      </ContentLayout>
    )
  }
      ...
      return (
        <div>
          <ContentLayout title='Danh sách bàn đã xóa'>
            <PageSupplier data={data} columns={columns} meta={res.data.meta} />
          </ContentLayout>
        </div>
      )
    }
  ...
  const res: IBackendRes<ISupplier> = await findSupplierById({ spli_id: id })
  ...
  return (
    <ContentLayout title='Chỉnh sửa thông tin nhà cung cấp'>
      <AddOrEdit id={id} inforSupplier={res.data} />
    </ContentLayout>
  )
}
