...
export function PageCategory<TData, TValue>({ columns, meta, data }: DataTableProps<TData, TValue>) {
  ...
  return (
    <div className='flex flex-col' style={{ height: 'calc(100vh - 7rem)' }}>
      <div className='flex justify-end gap-2 items-center py-4'>
        ...
        <Button variant={'outline'} disabled={!hasPermissionKey('category_blog_create')}>
          <Link href={'/dashboard/category-blog/add'}>Thêm</Link>
        </Button>
        {
          pathname === 'recycle' ? (
            <Button variant={'outline'} disabled={!hasPermissionKey('category_blog_view_list')}>
              <Link href={'/dashboard/category-blog'}>Danh sách</Link>
            </Button>
          ) : (
            <Button variant={'outline'} disabled={!hasPermissionKey('category_blog_view_deleted')}>
              <Link href={'/dashboard/category-blog/recycle'}>Danh sách đã xóa</Link>
            </Button>
          )
        }
