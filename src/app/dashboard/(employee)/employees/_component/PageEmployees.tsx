  ...
  return (
    <div className='flex flex-col' style={{ height: 'calc(100vh - 7rem)' }}>
      <div className='flex justify-end gap-2 items-center py-4'>
        ...
        <Button variant={'outline'} disabled={!hasPermission('employee_list_create')}>
          <Link href={'/dashboard/employees/add'}>Thêm</Link>
        </Button>
        {
          pathname === 'recycle' ? (
            <Button variant={'outline'} disabled={!hasPermission('employee_list_view_list')}>
              <Link href={'/dashboard/employees'}>Danh sách</Link>
            </Button>
          ) : (
            <Button variant={'outline'} disabled={!hasPermission('employee_list_view_deleted')}>
